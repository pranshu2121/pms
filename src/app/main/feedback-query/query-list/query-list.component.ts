...
export class QueryListComponent implements OnInit {
  ...
  private setval(data) {
    ...
    this.rows.map((data: any) => {
      ...
      data.requested_by.user_type = User_Type[data.requested_by.user_type],
         data.status_log.map(data=>data.status=Status[data.status]),
         data.currentStatus =  data.status_log[data.status_log.length-1].status
        ]
    })
  ...
  setlimit() {
    ...
    this.querylistcomman(this.query);
  }
  ...
  onSortFile(event) {
    if (event) {
      this.sortByColumn = event.sorts[0].prop + ":" + event.newValue
      let query = {
        search: this.searchBox.value,
        limit: this.limit,
        page: 1,
  ...
  filterByStatus(event: any) {
    this.tableOffset=0    
      this.query = {
        ...
        page: 1,
        sortBy: this.sortByColumn,
        status: event != undefined ? event?.value : event
      }
      this.querylistcomman(this.query)
  }
